...
import {
  ...
  Search, CheckCircle2, XCircle, FileText, Award, CreditCard,
} from 'lucide-react'
...
export function AccountantFullPortal() {
  const [tab, setTab] = React.useState<'dashboard' | 'debts' | 'collect' | 'receipts' | 'scholarships' | 'expenses'>('dashboard')
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
        <TabsList className="grid w-full grid-cols-3 sm:grid-cols-6 gap-1 h-auto">
          ...
          <TabsTrigger value="debts" className="flex flex-col items-center gap-1 py-2 text-xs">
            <AlertTriangle className="h-4 w-4" /> Dettes élèves
          </TabsTrigger>
                ...
                <tbody>
                  {receipts.map((r) => (
                    <tr key={r.id} className={`border-b hover:bg-muted/20 ${r.cancelled ? 'opacity-50' : ''}`}>
                      ...
                      <td className="p-2 text-center">
                        {!r.cancelled && (
                          <Button size="sm" variant="ghost" onClick={() => { setSelectedReceipt(r); setShowCancel(true) }}>
