...
// 14 comptes démo — affichés SEULEMENT sur le site démo (smart-shule-seven.vercel.app)
const DEMO_ACCOUNTS = [
...
// URLs des sites démo où les boutons de comptes démo sont affichés
const DEMO_SITE_URLS = [
  'smart-shule-seven.vercel.app',
  'localhost',
  '127.0.0.1',
]
...
export function LoginForm({ schoolName, schoolSlogan, schoolLogoUrl, primaryColor, secondaryColor }: { schoolName: string; schoolSlogan?: string; schoolLogoUrl?: string; primaryColor?: string; secondaryColor?: string }) {
  ...
  // Détecter si on est sur le site démo (afficher les boutons) ou sur l'app commerciale (page vierge)
  const [showDemoAccounts, setShowDemoAccounts] = React.useState(false)
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      const hostname = window.location.hostname
      const isDemo = DEMO_SITE_URLS.some(url => hostname.includes(url))
      setShowDemoAccounts(isDemo)
    }
  }, [])
          ...
          <div className="w-full max-w-md space-y-6 my-4">
            ...
            <Card className="ss-shadow-card">
              ...
              <CardContent className="space-y-4">
                ...
                {/* 14 comptes démo cliquables — UNIQUEMENT sur le site démo */}
                {showDemoAccounts && (
